...
def _validate_output_dir(output_dir: str | Path) -> Path:
    ...
    if path.name in PROTECTED_MODEL_DIRS:
        raise ValueError(f"Refusing to write Varaha 2 artifacts to protected model dir: {path}")
...
def train_timesplit_varaha_v2(
    ...
    print(
        "Training Varaha 2 HistGradientBoostingClassifier challenger on "
    ...
    metadata = {
        ...
        "warning": "time-split HGB challenger only; not deployed for live/demo trading",
    ...
    print("Varaha 2 time-split train-only HGB challenger completed successfully.")
    return metadata
