...
DEFAULT_MODEL_DIR = Path("/app/data/models/stock_opportunity_ohlcv_regime_timesplit_varaha_v3")
...
DEFAULT_OUTPUT_DIR = Path("/app/data/evaluations/stock_opportunity_ohlcv_regime_timesplit_varaha_v3")
...
MVP_PRECISION_THRESHOLD = 0.50
MVP_GATE_DEFINITION = (
    "precision >= 0.50 on full locked v3 test split at classification threshold 0.5"
)
...
def _validate_model_artifact_paths(
    ...
    for path, expected_name in expected_names.items():
        ...
        if protected_parts:
            raise ValueError(f"Refusing to load Varaha 3 artifact from protected old model dir: {path}")
        if path.name != expected_name:
            raise ValueError(f"Unexpected Varaha 3 artifact file name: {path}")
        if path.parent.name != MODEL_VERSION:
            raise ValueError(f"Varaha 3 artifacts must come from {MODEL_VERSION}: {path}")
        if not path.exists():
            raise FileNotFoundError(f"Varaha 3 artifact not found: {path}")
...
def _validate_output_dir(output_dir: str | Path) -> Path:
    ...
    protected_parts = sorted(set(path.parts).intersection(PROTECTED_MODEL_DIRS))
    if protected_parts:
        raise ValueError(f"Refusing to write Varaha 3 evaluation to protected dir: {path}")
...
def _validate_output_dir_contents(output_path: Path) -> None:
    ...
    if unexpected_files:
        raise ValueError(
            f"Refusing to write into Varaha 3 evaluation dir with unexpected files: {unexpected_files}"
        )
...
def load_feature_schema(schema_path: str | Path, expected_feature_count: int = 608) -> list[str]:
    ...
    _validate_feature_edges(data)
    return data


def _validate_feature_edges(feature_schema: list[str]) -> None:
...
def _validate_model_metadata(
    ...
    *,
    ...
    if mismatches:
        raise ValueError(f"Varaha 3 model metadata validation failed: {mismatches}")
    return metadata
...
def _validate_test_header(
    ...
    if len(header_columns) != expected_total_columns:
        raise ValueError(
            f"Total test column count must be exactly {expected_total_columns}, "
    ...
    _validate_feature_edges(feature_columns)
    return True
...
def _validate_dates(
    ...
    cutoff_date: str,
    expected_min_test_sample_date: str,
    expected_max_test_sample_date: str,
) -> tuple[str, str]:
    ...
    if max_test_sample_date != expected_max_test_sample_date:
        raise ValueError(
            "Max test sample_date does not match expected split boundary. "
            f"expected={expected_max_test_sample_date} actual={max_test_sample_date}"
        )
...
def _validate_model(model: Any) -> HistGradientBoostingClassifier:
    if not isinstance(model, HistGradientBoostingClassifier):
        raise ValueError(
            "Varaha 3 model must be HistGradientBoostingClassifier, got "
            f"{type(model).__name__}"
        )
    if not hasattr(model, "predict_proba"):
        raise ValueError("Varaha 3 model must support predict_proba")
    return model
...
def _build_metrics(
    ...
    return {
        ...
        "mvp_precision_threshold": MVP_PRECISION_THRESHOLD,
        "mvp_precision_gate_met": precision >= MVP_PRECISION_THRESHOLD,
        "mvp_gate_definition": MVP_GATE_DEFINITION,
    }


def score_timesplit_varaha_v3(
    ...
    expected_max_test_sample_date: str = "2026-05-18",
) -> tuple[dict, dict]:
    ...
    print(f"Loading Varaha 3 test-only dataset: {test_path}")
    ...
    min_test_sample_date, max_test_sample_date = _validate_dates(
        ...
        cutoff_date=cutoff_date,
        expected_min_test_sample_date=expected_min_test_sample_date,
        expected_max_test_sample_date=expected_max_test_sample_date,
    )
    ...
    print(f"Loading Varaha 3 model: {model_file}")
    ...
    print(f"Scoring Varaha 3 on {len(df)} test-only rows and {len(feature_schema)} features...")
    win_probabilities = model.predict_proba(X)[:, 1]
    predictions = (win_probabilities >= CLASSIFICATION_THRESHOLD).astype(np.int8)
    ...
    metadata = {
        ...
        "split_metadata_validated": True,
        "mvp_precision_threshold": MVP_PRECISION_THRESHOLD,
        "mvp_precision_gate_met": metrics["mvp_precision_gate_met"],
        "mvp_gate_definition": MVP_GATE_DEFINITION,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "git_commit": get_git_commit(),
        "warning": "Varaha 3 offline time-split evaluation only; not deployed for live/demo trading",
    }

    print(f"Saving score metadata to {metadata_path}")
    _atomic_write_json(metadata_path, metadata)

    print("Varaha 3 time-split test-only scoring completed successfully.")
