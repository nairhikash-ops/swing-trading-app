...
def run_track_shadow_shortlist(
    ...
    if not os.path.exists(csv_path) or not os.path.exists(meta_path):
        raise FileNotFoundError("Latest rankings or metadata not found.")
    ...
    print(f"Ranking rows: {total_rows}")
    print(f"Top 5% tracked: {top_5_count}")
    print(f"Primary Top 1%: {top_1_count}")
    print(f"Inserted: {inserted_count}")
    print(f"Skipped duplicates: {skipped_count}")
    print("Status: OBSERVING")

if __name__ == "__main__":
    run_track_shadow_shortlist()
