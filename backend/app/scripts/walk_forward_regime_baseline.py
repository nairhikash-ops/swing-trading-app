...
def run_regime_walk_forward_experiment(
    input_csv_path: str = "/app/data/exports/ml_dataset_ohlcv_regime_v1.csv",
    report_path: str = "/app/data/exports/regime_walk_forward_report.txt"
):
    ...
    actual_feature_cols = technical_cols + regime_cols
    
    # Validation checks
    missing_cols = set(actual_feature_cols) - set(df.columns)
    if missing_cols:
    ...
    report_lines = [
        "=== ML V1.7 REGIME WALK-FORWARD VALIDATION EXPERIMENT ===",
        "Artifact status: EXPERIMENTAL V1.7 DIAGNOSTICS - not for live trading",
    ...
    if aggregate_stats["number_of_periods"] > 0:
        ...
        delta_1 = avg_exp_1 - FROZEN_BASELINE_WF_AVG_EXP[1]
        delta_5 = avg_exp_5 - FROZEN_BASELINE_WF_AVG_EXP[5]
        ...
        report_lines.extend([
            ...
            "",
            f"Worst Top 1% period damage:    {aggregate_stats['worst_top_1_period']} (Exp: {aggregate_stats['worst_top_1_val']:+.4f})",
            f"Worst Top 5% period damage:    {aggregate_stats['worst_top_5_period']} (Exp: {aggregate_stats['worst_top_5_val']:+.4f})",
            "",
            f"Average Top 1% expectancy:     {avg_exp_1:+.4f} (vs Baseline: {FROZEN_BASELINE_WF_AVG_EXP[1]:+.4f} | Delta: {delta_1:+.4f})",
            f"Average Top 5% expectancy:     {avg_exp_5:+.4f} (vs Baseline: {FROZEN_BASELINE_WF_AVG_EXP[5]:+.4f} | Delta: {delta_5:+.4f})",
    ...
    print(f"Regime Walk-forward report saved to: {report_path}")

if __name__ == "__main__":
    run_regime_walk_forward_experiment()
