...
MODEL_VERSION = "stock_opportunity_ohlcv_regime_timesplit_varaha_v2"
MODEL_ALIAS = "Varaha 2"
...
def _is_train_csv_path(path: Path) -> bool:
    normalized_path = path.as_posix().rstrip("/")
    return normalized_path == FORBIDDEN_TRAIN_CSV.as_posix() or path.name == "train.csv"
...
def score_timesplit_varaha_v2(
    ...
    if _is_train_csv_path(test_path):
    ...
    print(f"Loading Varaha 2 model: {model_file}")
    ...
    print(f"Scoring Varaha 2 on {len(df)} test-only rows and {len(feature_schema)} features...")
    ...
    metadata = {
        ...
        "forbidden_train_csv": FORBIDDEN_TRAIN_CSV.as_posix(),
        ...
        "warning": (
            "offline time-split Varaha 2 challenger evaluation only; "
    ...
    print("Varaha 2 time-split test-only scoring completed successfully.")
