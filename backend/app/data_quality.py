...
class DataQualityService:
    ...
    def report(self, status_filter: str = "exceptions", limit: int = 200) -> dict[str, Any]:
        ...
        quality_items = [
            self._classify_item(dict(item), candles_by_item.get(item["id"], []), expected_sessions, expected_set, latest_expected)
            for item in items
        ]
    ...
    def _classify_item(
        ...
        if item["status"] not in ("skipped_unmapped", "failed"):
            if not candles:
                ...
                quality_status = "blocked"
            if latest_expected and latest_expected not in candle_dates:
