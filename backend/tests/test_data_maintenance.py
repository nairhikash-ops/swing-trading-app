...
@pytest.mark.asyncio
async def test_data_maintenance_fetches_without_pruning_by_default(tmp_path):
    historical = FakeHistoricalService({"status": "up_to_date", "id": 0})
    scheduler = DataMaintenanceScheduler(
        Settings(app_secret_key="a" * 44, data_dir=tmp_path),
    ...
    assert result["retention_status"] == "disabled"
    assert result["deleted_candle_count"] == 0
    assert historical.fetch_calls == 1
    assert historical.prune_calls == 0
    assert "regime_status" not in result
    assert "reversal_snapshot_status" not in result
    assert "demo_automation_status" not in result


@pytest.mark.asyncio
async def test_data_maintenance_prunes_only_when_explicitly_enabled(tmp_path):
    historical = FakeHistoricalService({"status": "running", "id": 10})
    scheduler = DataMaintenanceScheduler(
        Settings(app_secret_key="a" * 44, data_dir=tmp_path, auto_purge_market_data=True),
        FakeTokenService(active_token_status()),
        historical,
    )
    ...
    assert result["retention_status"] == "purged"
